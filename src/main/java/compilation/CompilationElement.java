...
public class CompilationElement {
	...
	private CompilationElement(){
		if(params == null ) params=new CompilationElement[0];
	}
	...
	public CompilationElement(String seq){
		...
		if(ch == '\"' || ch=='\''){
			this.type = '\"';
			base= seq.substring(1, seq.length()-1);		
		}
		else if(ch=='*'){
			this.type = '*';
	...
	public boolean equals(Object other){
		if(! (other instanceof CompilationElement)) return false;
		CompilationElement o = (CompilationElement) other;
		if(!Arrays.deepEquals(params, o.getParams())) return false;
