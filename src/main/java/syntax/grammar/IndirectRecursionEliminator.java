...
public class IndirectRecursionEliminator {
    ...
    private void pullUpRules(List<Stack<Rule>> branches, Map<String, ArrayList<Rule>> grammar, boolean strict)
            throws GrammarException {
        ...
        for (SyntaxElement rse : into.getRightside()) {
            ...
            boolean condition = rs != null && low.isDirectRecursive()
                    && low.containsRefOnRightSide(into.getGroupname());
            if (condition) {
                    ...
                    for (Rule r : branch) {
                        if (r.containsRefOnRightSide(into.getGroupname())) {
                            throw new GrammarException(
                                    "Cannot pull " + low + " into " + into + " because it is already recursive");
                        }
                    }
                }
            }
